...
LidarDatabaseExporter::~LidarDatabaseExporter()
{
  // base path
...
bool LidarDatabaseExporter::initialize_rtabmap_database()
{
...
void LidarDatabaseExporter::assembleSceneFromOptimizedPoses()
{
  ...
  for (std::map<int, rtabmap::Transform>::iterator iter =
    ...
    if (rgb.empty()) {
      std::cout << "Node " << iter->first << " missing rgb data, skipping.\n";
      continue;
    }

    if (depth.empty()) {
      std::cout << "Node " << iter->first << " missing depth data, skipping.\n";
      continue;
    }

    if (models.empty()) {
      std::cout << "Node " << iter->first
                << " missing models data, skipping.\n";
      continue;
    }
    ...
    rtabmap::Transform camViewpoint = iter->second * models[0].localTransform();
    rawViewpoints.insert(std::make_pair(iter->first, camViewpoint));
...
void LidarDatabaseExporter::projectAndColorizePointCloud()
{
  UTimer timer;
...
void LidarDatabaseExporter::assemble_colored_point_cloud()
{
  ...
  pointToCamId.resize(cloudToExport->size());
...
void LidarDatabaseExporter::finalize_and_return_result(Result &result)
{
...
LidarDatabaseExporter::project_cloud_to_camera(
